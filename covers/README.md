...
- **Tags & free-text notes** - tag a visit with cuisine or anything else (free-form chips that autocomplete from tags you've used) and jot notes; both are searchable, and tags are clickable to filter the list
- **Grouped by restaurant** - the list shows one row per restaurant (your latest visit) with a visit-count indicator under the date; click a row (not the edit controls) to expand previous visits in a condensed list
- **Repeat visits** - the **Log again** action pre-fills a new visit with that restaurant + tags + location; the name field autocompletes places you've been. The map also groups repeat visits per place
...
- **Search** across restaurant, location, dishes, tags, and notes
